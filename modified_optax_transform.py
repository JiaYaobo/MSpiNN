...
"""Replace the content below to the transform.py in optax"""
...
def scale_by_adam(
  ...
  """
    ...
    def init_fn(params):
        ...
        adam_weights = jax.tree_util.tree_map(jnp.zeros_like, params)
        return ScaleByAdamState(count=jnp.zeros([], jnp.int32), mu=mu, nu=nu, adam_weights=adam_weights)
    ...
    def update_fn(updates, state, params=None):
        ...
        adam_weights = jax.tree_util.tree_map(
            lambda v: 1 / (jnp.sqrt(v + eps_root) + eps), nu_hat)

        mu = utils.cast_tree(mu, mu_dtype)
        return updates, ScaleByAdamState(count=count_inc, mu=mu, nu=nu, adam_weights=adam_weights)
