...
def mse_loss(model, x, y):
    ...
    return pred, None, None, unpen_loss, unpen_grads


@eqx.filter_jit
def all_unpen_loss(model, x, y):
    unpen_loss, unpen_grads = grad_loss(model, x, y)
    rdg_loss, _ = ridge_loss(model)
    smooth_loss = rdg_loss + unpen_loss
    all_loss = smooth_loss + (1 - model.group_lasso_param) * \
        l1_reg(model) + model.group_lasso_param * l2_reg(model)
    return all_loss, smooth_loss, unpen_loss, unpen_grads
...
def all_pen_loss(model, x, y):
    ...
    all_loss = smooth_loss + (1 - model.group_lasso_param) * \
        l1_reg(model) + model.group_lasso_param * l2_reg(model)
    return pred, all_loss, smooth_loss, unpen_loss, grads