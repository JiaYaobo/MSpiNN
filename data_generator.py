...
class DataGenerator:
    ...
    def create_data(self, n_obs, xs):
        ...
        if self.dist == 0:
            eps = 1.0 * np.random.randn(n_obs, 1).reshape(n_obs, 1) + 0.0 * np.random.standard_t(1, n_obs).reshape(
                n_obs, 1)
        elif self.dist == 1:
            eps = .7 * np.random.randn(n_obs, 1).reshape(n_obs, 1) + .3 * np.random.standard_t(1, n_obs).reshape(n_obs,
                                                                                                                 1)
        else:
            eps = 0 * np.random.randn(n_obs, 1).reshape(n_obs, 1) + 1.0 * np.random.standard_t(1, n_obs).reshape(n_obs,
                                                                                                                 1)
        y = true_ys + 0.5 * eps
...
def load_ccle_data(fn_x='./data/CCLE/expression.csv', fn_y='./data/CCLE/drug.csv', dose_name='Paclitaxel'):
    df_x = pd.read_csv(fn_x)
    df_y = pd.read_csv(fn_y)
    dose = dose_name + '_ActArea'
    x = df_x.values[:, 1:101]
    y = np.expand_dims(df_y[dose].values, axis=1)
    data = np.hstack([x, y])
